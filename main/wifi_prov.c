...
// ─── LVGL event callback (runs inside lv_timer_handler, mutex held) ──────────
...
static void prov_reset(lv_color_t bg)
{
    ...
    // Load immediately so the clean dark bg is shown before content is added
    lv_scr_load(s_prov_scr);
}

static lv_obj_t *prov_label(const char *txt, lv_coord_t y_ofs)
{
    // Must be called with LVGL lock held, after prov_reset()
                           ...
                           bool password_mode, char *out, size_t out_sz)
{
    ...
    // Wait for user input (outside lock so LVGL task can run)
    while (!s_kb_done) {
...
void wifi_prov_task_fn(void *pvParameter)
{
    ...
    char ssid[64] = "";
    while (strlen(ssid) == 0) {
        if (!prov_keyboard("WiFi Network Name", "SSID", false,
                           ssid, sizeof(ssid))) {
            prov_status("Please enter SSID", lv_color_make(0x20, 0x10, 0x10));
    ...
    // ── Step 2: Password ──────────────────────────────────────────────────────
    char pass[64] = "";
    prov_keyboard("WiFi Password", "Password (leave blank if open)", true,
                  pass, sizeof(pass));
